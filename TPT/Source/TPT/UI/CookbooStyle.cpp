...
//macros used to help simplify the code inside Create()
//with marcos like this, we can use:
/*
	Style.Set("NormalButtonBrush", FButtonStyle().SetNormal(BOX_BRUSH("Button", FVector2D(54, 54), FMargin(14.0f / 54.0f))));
*/
//instead of
/*
	Style.Set("NormalButtonBrush", FButtonStyle().SetNormal(FSlateBoxBrush(FPaths::GameContentDir() / "slate" / "Button.png", FVector2D(54, 54), FMargin(14.0f / 54.0f))));
*/
...
#define OTF_FONT(RelativePath, ...) FSlateFontInfo(FPaths::GameContentDir() / "Slate" / RelativePath + TEXT(".otf"), __VA_ARGS_)
